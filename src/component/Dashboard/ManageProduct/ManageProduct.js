...
const ManageProduct = () => {
    ...
    useEffect(() => {
        fetch('https://shielded-refuge-42801.herokuapp.com/products')
            .then(res => res.json())
            .then(data => setProducts(data));
    }, [products])

    const handleDelete = (id) => {
        const url = `https://shielded-refuge-42801.herokuapp.com/products/${id}`
                ...
                products.map(product => <div key={product._id}>
                    ...
                    <div className=" my-order d-flex flex-column justify-content-between m-4 px-4 py-3 border-2 border-primary rounded-pill w-75 mx-auto">
                        ...
                        <div className="d-md-flex justify-content-center">
                            <div>
                                <button className="ms-0 btn border-0" style={{ backgroundColor: "rgba(245, 222, 179, 0.541)s" }} onClick={() => handleDelete(product._id)}>Delete</button>
                            </div>
                        </div>
                    </div>
                </div>)
