...
const ManageOrder = () => {
    ...
    useEffect(() => {
        fetch('https://shielded-refuge-42801.herokuapp.com/orders')
            .then(res => res.json())
    ...
    const handleApprove = data => {
        data.status = "shipped";
        if (data.email) {
            fetch(`https://shielded-refuge-42801.herokuapp.com/orders/${data._id}`, {
                method: "PUT",
                headers: {
    ...
    const handleDelete = (id) => {
        const url = `https://shielded-refuge-42801.herokuapp.com/orders/${id}`
    ...
    return (
        <div>
            <h2 className="fw-bold mt-3 text-primary">--Manage Orders--</h2>
            {
